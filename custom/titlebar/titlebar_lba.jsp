	...
	if (contentPage != null)
	{
		...
		if (strValue != "" && strValue != "<%=strSearch %>")
		{
			if (sapOnly.checked)
			{
				...
				var strQry = "SELECT distinct(r_object_id) FROM dm_document \
				where object_name in (select object_name from bhpb_pet_controlled_doc where any bhpb_pet_tag_no like upper ('%" + strValue + "%'))"; 
...
<!--MKM 30/11/09 modified for SAP search in UKPU-->
	<td>				
		<input type="checkbox" value="sap_only" id="checkbox1" title="This option will search for all documents linked to SAP functional locations">Search tags only
	</td>
</tr>
</table>
